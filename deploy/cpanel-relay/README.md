...
2. Configuración Django compartida validada con B4 y pruebas combinadas aprobadas.
...
- El paquete completo debe vivir fuera de `public_html` para no alterar ni exponer el sitio legado.
- Subdominio HTTPS dedicado; su `DocumentRoot` debe apuntar exactamente al directorio `public/`
  de ese paquete externo.
...
La integración B4+B14 declara estas variables en `backend/config/settings.py` y
`backend/.env.example`. La activación continúa pendiente de valores reales y autorización:
...
- Si la activación falla, restaurar en Render el backend Brevo previamente comprobado y confirmar un
  correo controlado; conservar el relay sin tráfico para diagnóstico y no borrar logs o estado hasta
  cerrar el incidente.
- Después del diagnóstico y sólo con autorización, eliminar paquete, subdominio y secreto. Nunca
  borrar ni modificar el sitio legado dentro de `public_html`.
