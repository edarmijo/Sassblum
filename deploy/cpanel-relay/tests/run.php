...
{
    return [
        'html_body' => '<p>Contenido privado</p>',
        'text_body' => 'Contenido privado',
...
$tests['validator accepts unordered exact contract'] = function (): void {
    ...
    assertSameValue('Ticket actualizado', $message->subject(), 'Subject must be preserved.');
...
$tests['audit log excludes message content and addresses'] = function () use (&$runtimeDir): void {
    ...
    assertTrueValue(strpos($log, 'Contenido privado') === false, 'Body must not be logged.');
